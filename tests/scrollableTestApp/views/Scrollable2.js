...
function(dom, domStyle, connect, lang, registry, at, TransitionEvent, Repeat, getStateful, Output){
	...
	return {
		...
		//	var repeatDom = dom.byId('repeatWidget');
			...
			connectResult = connect.connect(dom.byId('insert2x'), "click", function(){
				insertResult(repeatmodel.model.length-1);
			});
			_connectResults.push(connectResult);

			connectResult = connect.connect(dom.byId('insert10x2'), "click", function(){
			...
			connectResult = connect.connect(dom.byId('remove10x2'), "click", function(){
		...
		beforeActivate: function(){
			...
			if(dom.byId("back2") && this.app.isTablet){ 
				domStyle.set(dom.byId("back2"), "visibility", "hidden"); // hide the back button in tablet mode
			}
		},
